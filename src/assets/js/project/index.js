...
$(document).ready(function () {
    (async () => {
        ...
        try {
            const resp = await fetch('/employee/newall');
        ...
        table = $('#projectsTable').DataTable({
            ...
            columns: [
                ...
                {
                    data: "createdAt", render: date => new Date(date).toLocaleDateString("en-IN", {
                        day: "2-digit",
                        month: "2-digit",
                        year: "numeric",
                    })
    ...
    $('#projectsTable').on('click', '.editBtn', async function () {
        ...
        try {
            ...
            // Fetch employees
            const empRes = await fetch("/employee/newall");
    ...
    deleteProjectModal.addEventListener('show.bs.modal', function (event) {
        ...
        deleteProjectForm.action = `/projects/delete/${projectId}?_method=delete`;
    });
});